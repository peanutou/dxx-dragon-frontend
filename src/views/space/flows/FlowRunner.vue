<template>
    <n-split :max="0.8"
        ...
        <template #1>
            <!-- 左边内容 -->
            <n-card v-if="flowDefinition"
                    :title="flowDefinition.name"
                    size="small"
                    class="h-full w-full"
                ...
                <template #header-extra>
                    <div class="flex items-center space-x-2">
                        ...
                        <n-tag size="small"
                               type="info">{{ flowDefinition.status }}</n-tag>
                        <n-tag size="small"
                               type="info">{{ `版本：${flowDefinition.version}` }}</n-tag>
                    </div>
                </template>
                <template #default>
                    <n-thing class="mb-4">{{ flowDefinition.description }}</n-thing>
                    <!-- Run Histroy Items -->
                    <n-form :model="{ inputValues }"
                            label-width="80"
                            class="space-y-0">
                        <n-form-item label="运行历史">
                            <n-select v-if="flowDefinition.test_inputs_history"
                                      v-model:value="selectedRunId"
                                      :options="flowDefinition.test_inputs_history.map((item: any, index: number) => ({
                                        label: `#${index + 1} ｜${new Date(item.run_at).toLocaleString()}`,
                                        value: item.run_id
                                    }))"
                                      placeholder="选择一次历史运行"
                                      style="width: 100%;">
                            </n-select>
                        </n-form-item>
                        <!-- Seperator -->
                        <n-divider title-placement="center"
                                   style="margin-top: 0px; font-size: 12px;">全局输入</n-divider>
                        <flow-inputs v-if="flowDefinition"
                                     :inputs="flowDefinition?.config?.inputs || []"
                                     v-model:inputValues="inputValues" />
                    </n-form>
                    <!-- execute result: outputs.trace -->
                    <n-button v-if="flowResult"
                              type="warning"
                              size="small">
                    ...
                    <n-data-table v-if="nodeResults"
                                  :columns="nodeResultColumns"
                                  :data="Object.values(nodeResults)"
                                  size="small"
                                  class="mt-4 text-xs" />
                </template>
                <template #action>
...
import {
    NCard, NTag, NButton, NTooltip, NDivider, NDataTable,
    NSplit, NForm, NSelect, NFormItem, NThing
} from 'naive-ui'
...
async function fetchFlow() {
    try {
        ...
        // 填充 inputValues 的初始值，boolean 类型为 false
            ...
            flowDefinition.value.config.inputs.forEach((field: any) => {
                if (!(field.name in inputValues.value)) {
                    if (field.type === 'number') {
                        inputValues.value[field.name] = 0;
                    } else if (field.type === 'boolean') {
                        inputValues.value[field.name] = false;
                    } else if (field.type === 'select') {
                        inputValues.value[field.name] = field.options?.[0]?.value || '';
...
async function handleRun() {
    ...
    try {
        ...
        if (testMode) nodeResults.value = res.data?.data?.context_list[0]?.node_results || null
