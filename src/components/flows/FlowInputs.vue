<template>
    <n-form label-placement="top" label-width="280px" :model="inputValues">
        <n-form-item v-for="field in inputs" :key="field.name" :path="field.name">
            <template #label>
            ...
            <div class="flex items-center gap-2 w-full" v-if="field.type === 'string'">
                <n-input v-model:value="inputValues[field.name]" :placeholder="`请输入 ${field.label || field.name}`" />
                <n-button quaternary circle size="tiny" @click="inputValues[field.name] = ''">×</n-button>
            </div>
            <div class="flex items-center gap-2 w-full" v-else-if="field.type === 'number'">
                <n-input-number v-model:value="inputValues[field.name]" :placeholder="`请输入数字`" />
                <n-button quaternary circle size="tiny" @click="inputValues[field.name] = undefined">×</n-button>
            </div>
            <div class="flex items-center gap-2 w-full" v-else-if="field.type === 'select'">
            ...
            <div class="flex items-center gap-2 w-full" v-else-if="field.type === 'boolean'">
